...
class EtoroHttpClient {
    ...
    fun getPersonHistory(mode: TradingMode, cid: String): YearMonthPerformance {
        val req = prepareRequest(
            "sapi/userstats/gain/cid/${cid}/history?IncludeSimulation=true&&client_request_id=${userContext.requestId}",
            userContext.exchangeToken, mode, metadataService.getMetadata()
        )
            .GET()
            .build()
    ...
    fun getPersonDailyChartData(mode: TradingMode, userName: String, period: String?): DailyChartData {
        val req = prepareRequest(
            "sapi/userstats/CopySim/Username/${userName}/${if (period != null) period else "OneYearAgo"}?callback=angular.callbacks._1&client_request_id=${userContext.requestId}",
            userContext.exchangeToken, mode, metadataService.getMetadata()
        )
            .GET()
            .build()
    ...
    fun getPortfolioShares(mode: TradingMode, cid: String): List<MirrorWithPositions> {
        val response = JSONObject(getAllLoginData(mode))
        ...
        val positions: List<MirrorWithPositions> = mapper.readValue(response)
